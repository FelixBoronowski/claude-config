...
3. Nothing else. Never paraphrase failure messages; never claim green without both a `0 failures` summary line and exit code 0. A run that ends without a summary line (aborted, crashed, timed out) is a failure: report the exit code and the last 20 lines verbatim.
